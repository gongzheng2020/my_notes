...
4. Q：CPU中断响应流程？
A：当发生中断后，首先就是保存现场，将寄存器压入栈中，再从中断向量表中取入口地址装入PC中，开始执行中断函数，执行完毕后恢复现场，寄存器出栈并恢复PC指针，回到主程序继续运行。

5. Q：中断中执行耗时操作的影响？
A：会影响后台程序的实时性（主程序代码不能及时运行、同级或低优先级中断被阻塞）；导致系统看门狗复位。

6. Q：介绍一下bootloader？介绍一下构建思路。
A：bootloader其实就是一段启动程序，它在芯片启动的时候首先被执行，它可以用来做一些硬件的初始化、固件升级等操作，当操作完成之后跳转到对应的应用程序中去。
大概思路为，Flash 分区 → 升级判断 → 固件下载 → 固件校验 → APP 跳转
补充：如何实现跳转？直接定义一个函数指针变量，将APP入口地址转换成函数指针类型后赋值给这个变量，再执行就能跳转。

7. GPIO 中断假如有很多个，怎么判断是哪一个
A：GPIO中断也就是外部中断，多个引脚挂在同一根中断向量线上，所以在触发之后需要进入中断服务函数中之后，通过读取EXIT的挂起寄存器(EXIT→PR)并且按位与&操作判断到底是哪一位触发了中断，处理完之后该位必须写1清零。
补充:PR相关知识
